	...
	internal sealed partial class Parser
	{
		...
		private Item SimpleMatch(Capture match, ref int i, int len)
		{
			...
			if (match.Type == IdenType.Constant)
			{
				...
				Output.PutNumber(Context.Constants[match.Index]);
				return Item.Identifier;
			}
			...
			if (match.Type == IdenType.Function)
			{
				...
				if (func == null)
				{
					...
					throw WrongArgsCount(funcPos, len, argsCount, group);
				}

				Output.PutFunction(func, argsCount);
				return Item.End;
			}
		...
		private Item AmbiguousMatch(List<Capture> matches, ref int i, int len)
		{
			...
			Output.PutFunction(overloads[0], argsCount);
			return Item.End;
		}
		...
		private int ArgsLookup(int fromPos)
		{
			int depth = 1, args = 0;
			string ex = this.expr;
			...
			for (int i = fromPos; i < ex.Length; i++)
			{
				...
				else
				{
					...
					{
						if (depth == 1 && c == this.sepSymbol) args++;
					}
				}
			}
		...
		private List<Capture> GetMatchesCulture(ref int max)
		{
			...
			foreach (IQuickEnumerable list in Context.Literals)
			{
				int id = 0;
