	...
	static class Benchmarks
	{
		...
		public static void EvaluateOnceTest()
		{
			Tester.Run(
				...
				() => {
					var flee = new ExpressionContext();
					flee.CompileGeneric<int>("2+2").Evaluate();
					return "FLEE";
				},
				() => {
					new Expression("2+2").Evaluate();
					return "NCalc";
				},
				() => {
					var ilCalc = new CalcContext();
					ilCalc.CreateEvaluator("2+2").Evaluate();
					return "ILCalc Eval";
				},
				() => {
					var ilCalc = new CalcContext();
					ilCalc.CreateInterpret("2+2").Evaluate();
		...
		public static void ILCalcOptimizerTest()
		{
			...
			Tester.Run(
				...
				() => { eval0.Evaluate(1.23, 4.56); return "Evaluator"; },
				() => { eval1.Evaluate(1.23, 4.56); return "+ConstFolding"; },
				() => { eval2.Evaluate(1.23, 4.56); return "+PowOptimize"; },
				() => { eval3.Evaluate(1.23, 4.56); return "+FuncFolding"; },
				() => { intr0.Evaluate(1.23, 4.56); return "Interpret"; },
				() => { intr1.Evaluate(1.23, 4.56); return "+ConstFolding"; },
				() => { intr2.Evaluate(1.23, 4.56); return "+PowOptimize"; },
				() => { intr3.Evaluate(1.23, 4.56); return "+FuncFolding"; }
				);
		}
		...
		public static void TabulationTest()
		{
			...
			var range = new ValueRange(0, 1000000, 1);
		...
		private static double[] FleeTabHelper(
			IGenericExpression<double> expr, ValueRange range)
		{
		...
		private static double[] InterpretTabHelper(Interpret eval, ValueRange range)
		{
			Debug.Assert(range.IsValid);
		...
		private static double[] EvaluatorTabHelper(Evaluator eval, ValueRange range)
		{
			Debug.Assert(range.IsValid);
